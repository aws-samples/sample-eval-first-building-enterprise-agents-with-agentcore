...
4. [与 Eval-First 方法论的对应](#-与-eval-first-方法论的对应)
5. [前置条件](#2-前置条件)
6. [执行顺序速览](#3-执行顺序速览)
7. [逐步说明](#4-逐步说明)
8. [可选实验](#-可选实验)
9. [清理](#5-清理--99-cleanupsh)
10. [数据来源与署名](#6-数据来源与署名)
...
在 **`TRACE`** 级别运行（即*玻璃盒*粒度——它检视一条执行轨迹）。把一次问答拆成 `(问题, 检索到的来源, 回答)`。THELMA 论文定义了 **6 项指标**；本实现把 **Source Precision 拆成两个分数**上报（chunk 级 vs. fact 级），所以你每条 trace 会看到 **7 个数**（都是 0–1）：
...
> [!NOTE]
> **TRACE → 玻璃盒**、**SESSION → 黑盒** 这个对应是刻意的：AgentCore 的 session / trace / span 三个层级，正好对上配套白皮书里的三种评估粒度（黑盒 / 玻璃盒 / 白盒）。在那套框架里，这两个评估器都是**自定义的 L2 评估器**（经校准的 LLM-as-a-judge）——见[下文](#-与-eval-first-方法论的对应)。
...
这个 sample 是一套四篇《企业生产级智能体开发部署指南》白皮书的**动手姊妹篇**。白皮书讲*为什么*和方法论框架，这个 repo 让你把它端到端跑一遍。对应关系：
...
| **三层证据权重** —— L1 代码 / L2 校准过的 LLM-judge / L3 默认拒评 | THELMA 和 Mind the Goal 都是**自定义 L2 评估器**；`13-judge-stability.sh` 检验 L2 可信度，人工 TPR/TNR 校准是其 L3 补充 |
...
> [!TIP]
> `10-optimize-prompt.sh` 是一个**手动**的「优化并重新评估」循环。AgentCore **Optimization**（public preview）把同一个思路产品化了——在 AgentCore Evaluations 之上提供 Recommendations、版本化的 Configuration bundles 和 A/B testing。这里的手动循环就是它背后的概念原型。
...
- `thelma_rag_quality` —— **TRACE** 级，RAG 质量（7 个分数，见上方指标表），主分 = **Groundedness（有据性）**
...
默认会跑 **3 个 golden 问题**（绩效评估 / 福利 / 病假）产出 trace，等它们索引完成，再评估这些 trace，并为每条打印：**Query**、截断后的 **Response**、以及**分数**（THELMA 7 个分数拆解 + 诊断，外加 Mind the Goal 的 GSR + RCOF）。
